...
const DB = {
  ...
  shareTokens:    new Map(),   // shareToken → postId
  reports:        new Map(),
  notices:        new Map(),   // 관리자 공지
  noticesList:    [],
};
...
app.post('/api/community/posts', (req, res) => {
  ...
  const post = { id, analysisId, verdict, avgConfidence, maxConfidence, totalFrames, suspiciousSegments:suspiciousSegments||[], scores:scores||[], fileName, inputType, sourceUrl, hash, memo: (memo||'').slice(0,500), mediaData: mediaData||null, shareToken, postedAt: new Date().toISOString(), flagCount:0, status:'ACTIVE' };
...
/* ─── 신고된 커뮤니티 게시물 삭제 (관리자) ──────────────────── */
app.delete('/api/admin/community/:id', requireAuth, (req, res) => {
  const p = DB.communityPosts.get(req.params.id);
  if (!p) return res.status(404).json({ error: 'Not found' });
  p.status = 'DELETED';
  res.json({ ok: true });
...
/* ─── 신고 상세 업데이트 (기존 patch 보강) ───────────────────── */
// 기존 /api/admin/reports/:id PATCH 에 delete action 추가 처리는
// 아래 별도 DELETE 라우트로 처리
app.delete('/api/admin/reports/:id', requireAuth, (req, res) => {
  if (!DB.communityPosts.has(req.params.id)) return res.status(404).json({ error: 'Not found' });
  DB.communityPosts.delete(req.params.id);
  res.json({ ok: true });
