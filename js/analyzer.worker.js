...
function blend(h,g) {
  ...
  return {df: gDf*0.92 + h.df*0.08,
          ai: gAi*0.92 + h.ai*0.08,
          _signals:h._signals};
}
...
async function analyzeSegment({frames}) {
  ...
  // effectiveAvg: Gemini verdict 기준으로 최종 평균 결정
  let effectiveAvg;
  if(geminiResult){
    ...
    if(gAiR && gDfR){
      // 양쪽 다 실제 → 매우 낮은 점수 (blend된 combined 평균 사용)
      effectiveAvg = Math.min(cAvg, 0.20);
    } else {
      // 조작 신호가 있을 때만 높게
      effectiveAvg = Math.max(dfAvg, aiAvg);
    }
  } else {
    effectiveAvg = isImage ? Math.max(cAvg, aiAvg*0.95, dfAvg*0.9) : cAvg;
  }
